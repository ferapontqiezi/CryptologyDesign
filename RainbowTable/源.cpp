	...
	for (int i = 0; i < m; i++) {
		if (strcmp(str, endStr[i]) == 0) {
			...
			for (int j = 0; j < 100000; j++) {
				UnitSHA1(strTemp, 8, sha1Temp);		// �õ�sha1
				for (k = 0; k < 5; k++) {
				...
				R(sha1Temp, strTemp, j % 100 + 1);	// �õ�str
			}
		}
	}
	return false;
...
int main() {
	freopen("E:\\CryptologyDesgin\\RainbowTable\\1.in", "r", stdin);
	...
	for (int i = 0; i < 100; i++) {
		...
		for (int j = 0; j < (i <= 1?100000:10000); j++) {
			R(sha1Temp, str, (i + j) % 100 + 1);	// �õ�str
			findFlag = findStr(str, sha1);
			if (findFlag)
				break;
			UnitSHA1(str, 8, sha1Temp);		// �õ�sha1
		}
		if (findFlag)
			break;
	}
	...
	//	UnitSHA1(str, 8, sha1);		// �õ�sha1
	//	R(sha1, str, i%100 + 1);	// �õ�str
	//}
	return 0;
}